...
// runnng this command
//
//   slabgen -inpkg ./blktypes -outdir ./blocks SuperBlock
//
// will create file superblock.slab.go, in ./blocks directory containing
...
import (
	...
	"github.com/visvasity/slabgen/typecheck"
...
func Usage() {
	fmt.Fprintf(os.Stderr, "Usage of slabgen:\n")
	fmt.Fprintf(os.Stderr, "\tslabgen -inpkg '...' -outpkg '...' -outdir '...' types... # Must be a single package\n")
...
func main() {
	...
	for _, typ := range g.GetTypes() {
		...
		// Write to file.
		outputName := filepath.Join(*outDir, strings.ToLower(typ)+".slab.go")
...
type Generator struct {
	...
	//
	// in the generated file named "point.slab.go".
...
func (g *Generator) getSourceWithImports(typeName string) *bytes.Buffer {
	...
	fmt.Fprintln(buf, "// Code generated by github.com/visvasity/slabgen. DO NOT EDIT.")
	fmt.Fprintln(buf)
...
func (g *Generator) prepFieldTypeName(sdata *StructData, fdata *typecheck.FieldData, prefix, suffix string, opts *fieldTypeNameOptions) string {
	...
	if opts.IncludeReaderWriterTypeArgs {
		for i, ta := range fdata.TypeArgs {
			if fdata.TypeParams[i].Constraint == "Struct" && fdata.TypeParams[i].PkgPath == "github.com/visvasity/slabgen/slabs" {
...
func (g *Generator) generateReaderWriterTypes(sdata *StructData) error {
	if err := g.addImport(sdata.StructName, "", "github.com/visvasity/slabgen/slabs"); err != nil {
		return err
	}
	...
	g.P(sdata.StructName, "type ", readerDefinition, " []byte")
	g.P(sdata.StructName)
...
func (g *Generator) generateReaderWriterMethods(sdata *StructData) error {
	...
	g.P(typeName)
	g.P(typeName, "// SlabBytes returns access to the underlying byte slice.")
	g.P(typeName, "func (v ", readerReceiver, ") SlabBytes() slabs.Bytes {")
	g.P(typeName, "  return []byte(v)")
	g.P(typeName, "}")
	g.P(typeName)
...
func (g *Generator) generateFieldMetadataVars(sdata *StructData) error {
	...
	if !g.IsGenericStruct(sdata) {
		if err := g.addImport(typeName, "", "github.com/visvasity/slabgen/slabutil"); err != nil {
			return err
		}
		...
		g.P(typeName)
		g.P(typeName, "var ", g.structSizeVarName(sdata), " = slabutil.SizeFor[", inputStructName, "]()")
		g.P(typeName, "var ", g.fieldOffsetsVarName(sdata), " = slabutil.OffsetsFor[", inputStructName, "](nil)")
		...
		if sfdata := g.sliceField(sdata); sfdata != nil {
			...
			g.P(typeName, "var ", g.sliceDataAlignVarName(sdata), " = slabutil.AlignFor[[]", sliceFieldTypeName, "]()")
			g.P(typeName, "var ", g.sliceElementSizeVarName(sdata), " = slabutil.ElemSizeFor[[]", sliceFieldTypeName, "]()")
		}
		g.P(typeName)
		return nil
	}
	...
	g.P(typeName)
	g.P(typeName, "var ", g.fieldOffsetsMapVarName(sdata), " slabutil.OffsetsMap")
	g.P(typeName)
	return nil
}
...
func (g *Generator) generateZeroMethods(sdata *StructData) error {
	...
	g.P(typeName, "  return slabutil.IsZero(v[:", g.structSizeVarName(sdata), "])")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName, "  slabutil.SetZero(v.SlabBytes()[:", g.structSizeVarName(sdata), "])")
	g.P(typeName, "}")
	g.P(typeName)
	return nil
}
...
func (g *Generator) generateNumberFieldMethods(sdata *StructData, fdata *typecheck.FieldData) error {
	...
	if !g.IsGenericStruct(sdata) {
		...
		g.P(typeName, "  return ", fieldTypeName, "(v.SlabBytes().", fmethods[1], "(offset))")
		g.P(typeName, "}")
		g.P(typeName)
	} else {
		g.P(typeName)
		g.P(typeName, "func (v ", readerReceiver, ") ", fdata.FieldName, "() ", fieldTypeName, " {")
		g.P(typeName, "  var ", g.fieldOffsetsVarName(sdata), " = slabutil.OffsetsFor[", inputStructName, "](&", g.fieldOffsetsMapVarName(sdata), ")")
		g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "]")
		if fdata.BasicKind == "generic" {
			g.P(typeName, "  return slabutil.NumberAt[", fieldTypeName, "](v.SlabBytes(), offset)")
		} else {
			g.P(typeName, "  return ", fieldTypeName, "(v.SlabBytes().", fmethods[1], "(offset))")
		}
		g.P(typeName, "}")
	...
	if !g.IsGenericStruct(sdata) {
		...
		g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "]")
		g.P(typeName, "  v.SlabBytes().", fmethods[2], "(offset, ", fmethods[0], "(x))")
		g.P(typeName, "}")
		g.P(typeName)
	} else {
		g.P(typeName)
		...
		g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "]")
		if fdata.BasicKind == "generic" {
			g.P(typeName, "  slabutil.SetNumberAt[", fieldTypeName, "](v.SlabBytes(), offset, x)")
		} else {
			g.P(typeName, "  v.SlabBytes().", fmethods[2], "(offset, ", fmethods[0], "(x))")
		}
		g.P(typeName, "}")
...
func (g *Generator) generateStructFieldMethods(sdata *StructData, fdata *typecheck.FieldData) error {
	...
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "]")
	g.P(typeName, "  return ", fieldReaderTypeName, "(v.SlabBytes()[offset:])")
	g.P(typeName, "}")
	g.P(typeName)
	return nil
}
...
func (g *Generator) generateArrayOfNumbersFieldMethods(sdata *StructData, fdata *typecheck.FieldData) error {
	...
	g.P(typeName, "  var elemSize = slabutil.ElemSizeFor[[1]", fieldTypeName, "]()")
...
func (g *Generator) generateArrayOfStructsFieldMethods(sdata *StructData, fdata *typecheck.FieldData) error {
	...
	g.P(typeName, "  var elemSize = slabutil.ElemSizeFor[[1]", fieldReaderTypeName, "]()")
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "] + i * elemSize")
	g.P(typeName, "  return ", fieldReaderTypeName, "(v.SlabBytes()[offset:])")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName, "  var elemSize = slabutil.ElemSizeFor[[1]", fieldReaderTypeName, "]()")
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", fdata.Index, "] + i * elemSize")
	g.P(typeName, "  copy(v.SlabBytes()[offset:offset+elemSize], x[:elemSize])")
	g.P(typeName, "}")
	g.P(typeName)
...
func (g *Generator) generateSliceMethods(sdata *StructData) error {
	...
	if g.IsGenericStruct(sdata) {
		g.P(typeName, "  var ", g.structSizeVarName(sdata), " = slabutil.SizeFor[", inputStructName, "]()")
		g.P(typeName, "  var ", g.sliceElementSizeVarName(sdata), " = slabutil.ElemSizeFor[", fieldTypeName, "]()")
	}
	...
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", sfdata.Index, "] + slabutil.OffsetOfSliceLen")
	g.P(typeName, "  return slabutil.IntAt(v.SlabBytes(), offset)")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", sfdata.Index, "] + slabutil.OffsetOfSliceLen")
	g.P(typeName, "  slabutil.SetIntAt(v.SlabBytes(), offset, x)")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName, "  var offset = ", g.fieldOffsetsVarName(sdata), "[", sfdata.Index, "] + slabutil.OffsetOfSliceCap")
	g.P(typeName, "  return slabutil.IntAt(v.SlabBytes(), offset)")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName)
	g.P(typeName, "  copy(v.SlabBytes()[ioff:end], v.SlabBytes()[joff:end])")
	g.P(typeName, "  slabutil.SetZero(v.SlabBytes()[end-(joff-ioff):end])")
...
func (g *Generator) generateSliceOfStructsFieldMethods(sdata *StructData, fdata *typecheck.FieldData) error {
	...
	g.P(typeName, "  var elemSize = slabutil.ElemSizeFor[[]", fieldTypeName, "]()")
	g.P(typeName, "  var offset = ", g.structSizeVarName(sdata), " + i * elemSize")
	g.P(typeName, "  copy(v.SlabBytes()[offset:offset+elemSize], x.SlabBytes()[:elemSize])")
	g.P(typeName, "}")
	g.P(typeName)
	...
	g.P(typeName, "    slabutil.SetZero(v.SlabBytes()[offset:offset+elemSize])")
	g.P(typeName, "  } else {")
	g.P(typeName, "    copy(v.SlabBytes()[offset:offset+elemSize], x.SlabBytes()[:elemSize])")
	g.P(typeName, "  }")
	g.P(typeName, "}")
...
func (g *Generator) generateNewAndOpenMethods(typeName string) error {
	...
	g.P(typeName, "  }")
	g.P(typeName, "  slabutil.SetZero(block)")
