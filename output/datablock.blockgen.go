...
type DataBlockReader[T blockgen.Struct, TReader blockgen.Reader[T, TWriter], TWriter blockgen.Writer[T, TReader]] blockgen.BlockBytes

// Writer type extends the reader with mutable methods.
type DataBlockWriter[T blockgen.Struct, TReader blockgen.Reader[T, TWriter], TWriter blockgen.Writer[T, TReader]] struct {
	DataBlockReader[T, TReader, TWriter]
}

var fieldOffsetsOfDataBlock blockgen.OffsetsMap

// BlockBytes returns access to the underlying byte slice.
func (v DataBlockReader[T, TReader, TWriter]) BlockBytes() blockgen.BlockBytes {
...
func (v DataBlockReader[T, TReader, TWriter]) IsZero() bool {
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	return blockgen.IsZero(v[:structSizeOfDataBlock])
}

// SetZero sets all underlying bytes to zero.
func (v DataBlockWriter[T, TReader, TWriter]) SetZero() {
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	blockgen.SetZero(v.BlockBytes()[:structSizeOfDataBlock])
}
...
func (v DataBlockReader[T, TReader, TWriter]) NextLBA() storage.LBA {
	var fieldOffsetsOfDataBlock = blockgen.OffsetsFor[input.DataBlock[T]](&fieldOffsetsOfDataBlock)
	var offset = fieldOffsetsOfDataBlock[0]
	return storage.LBA(v.BlockBytes().Uint64At(offset))
}
...
func DataBlockSliceFieldCap[T blockgen.Struct](nbytes int) int {
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	var sliceElemSizeOfDataBlock = blockgen.ElemSizeFor[T]()
...
func (v DataBlockReader[T, TReader, TWriter]) ValuesSliceLen() int {
	var fieldOffsetsOfDataBlock = blockgen.OffsetsFor[input.DataBlock[T]](&fieldOffsetsOfDataBlock)
	var offset = fieldOffsetsOfDataBlock[1] + blockgen.OffsetOfSliceLen
...
func (v DataBlockReader[T, TReader, TWriter]) ValuesSliceCap() int {
	var fieldOffsetsOfDataBlock = blockgen.OffsetsFor[input.DataBlock[T]](&fieldOffsetsOfDataBlock)
	var offset = fieldOffsetsOfDataBlock[1] + blockgen.OffsetOfSliceCap
	return blockgen.IntAt(v.BlockBytes(), offset)
}
...
func (v DataBlockWriter[T, TReader, TWriter]) ResizeValuesSlice(size int) int {
	...
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	var elemSize = blockgen.ElemSizeFor[[]T]()
...
func (v DataBlockReader[T, TReader, TWriter]) ValuesSliceItemAt(i int) TReader {
	...
	return TReader(v.BlockBytes()[offset:])
}
...
func NewDataBlock[T blockgen.Struct, TReader blockgen.Reader[T, TWriter], TWriter blockgen.Writer[T, TReader]](block []byte) DataBlockReader[T, TReader, TWriter] {
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	size := len(block)
...
func OpenDataBlock[T blockgen.Struct, TReader blockgen.Reader[T, TWriter], TWriter blockgen.Writer[T, TReader]](block []byte) (DataBlockReader[T, TReader, TWriter], error) {
	var structSizeOfDataBlock = blockgen.SizeFor[input.DataBlock[T]]()
	size := len(block)
	if size < structSizeOfDataBlock {
