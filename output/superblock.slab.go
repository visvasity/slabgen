...
type SuperBlockReader []byte

// Writer type extends the reader with mutable methods.
type SuperBlockWriter struct{ SuperBlockReader }

var structSizeOfSuperBlock = slabutil.SizeFor[input.SuperBlock]()
var fieldOffsetsOfSuperBlock = slabutil.OffsetsFor[input.SuperBlock](nil)
var sliceDataAlignOfSuperBlock = slabutil.AlignFor[[]input.JournalRegion]()
var sliceElemSizeOfSuperBlock = slabutil.ElemSizeFor[[]input.JournalRegion]()

// SlabBytes returns access to the underlying byte slice.
func (v SuperBlockReader) SlabBytes() slabs.Bytes {
	return []byte(v)
}
...
func (v SuperBlockReader) IsZero() bool {
	return slabutil.IsZero(v[:structSizeOfSuperBlock])
}

// SetZero sets all underlying bytes to zero.
func (v SuperBlockWriter) SetZero() {
	slabutil.SetZero(v.SlabBytes()[:structSizeOfSuperBlock])
}
...
func (v SuperBlockReader) MaxObjectID() input.ObjectID {
	...
	return input.ObjectID(v.SlabBytes().Int64At(offset))
}
...
func (v SuperBlockReader) DBARegionList() LinkedListReader {
	...
	return LinkedListReader(v.SlabBytes()[offset:])
}
...
func (v SuperBlockReader) JournalRegionSliceLen() int {
	var offset = fieldOffsetsOfSuperBlock[11] + slabutil.OffsetOfSliceLen
...
func (v SuperBlockReader) JournalRegionSliceCap() int {
	var offset = fieldOffsetsOfSuperBlock[11] + slabutil.OffsetOfSliceCap
	return slabutil.IntAt(v.SlabBytes(), offset)
}
...
func (v SuperBlockWriter) ResizeJournalRegionSlice(size int) int {
	...
	var elemSize = slabutil.ElemSizeFor[[]input.JournalRegion]()
...
func (v SuperBlockReader) JournalRegionSliceItemAt(i int) JournalRegionReader {
	...
	return JournalRegionReader(v.SlabBytes()[offset:])
}
